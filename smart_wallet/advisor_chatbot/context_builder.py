...
def build_financial_context(user):
    ...
    if transactions.exists():
        for t in transactions[:20]:   # cap at 20 to stay within token limits
            lines.append(
                f"  [{t.type.upper()}] {t.category.name} | "
    ...
    if cat_totals.exists():
        lines.append("\n=== EXPENSE BY CATEGORY (THIS MONTH) ===")
        for row in cat_totals:
            lines.append(f"  {row['category__name']}: {row['total']}")
    ...
    if budgets.exists():
        ...
        for b in budgets:
            ...
            lines.append(
                f"  {b.category.name}: spent {b.spended} / limit {b.amount} [{status}]"
            )
